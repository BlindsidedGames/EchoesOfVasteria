- Fixed Forge autocrafting causing unnecessary Cauldron GC allocations: added SalvageService.BatchSalvageWithinBatch() to coalesce batch salvage into existing ResourceManager batch (eliminates double OnInventoryChanged events), added visibility check to CauldronWindowUI.OnInventoryChangedUi() using TownWindowManager.IsCauldronOpen, implemented dirty flag throttling with LateUpdate to coalesce multiple inventory events per frame, cached eligible foods list with invalidation on window open/save load to avoid repeated LINQ allocations. Combined effect: ~100KB GC per craft batch → near zero when Cauldron closed, ~5KB when open.
- Fixed major GC allocation hotspot in CauldronManager.IsIdMaxed() and BuildResourceIdsForGroup(): added cached CardId properties to Resource and BuffRecipe ScriptableObjects, refactored CardTierCalculator.IsMaxed() to avoid Substring allocations by using first-char prefix check and direct dictionary lookup, updated CauldronManager to use cached CardId instead of string interpolation. Eliminates ~85KB/frame of allocations during tasting (671 calls × Object.get_name + Substring + string concat).
- Added StringBuilder caching to BuffUIManager with per-slot dirty checking to reduce GC allocations from RefreshSlots() - now skips unchanged slots and uses StringBuilder for percent text display instead of string interpolation.
- Added StringBuilder caching to GeneralStatsPanelUI.UpdateTexts() to eliminate string interpolation allocations from stats display.
- Added StringBuilder caching to CauldronWindowUI.RefreshDrinkingTexts() and OnSessionCardsChanged() to eliminate string interpolation allocations from stew/Eva/XP text updates.
...
- Fixed AEResourceGroupClassifier causing ~100KB/frame GC allocations during Cauldron operations: removed unnecessary _groupClassifier.ClearCache() call from OnInventoryChangedHandler (resource groups are static and don't depend on inventory state), added static TaskData→AEResourceGroup cache to InferGroupFromTask to avoid repeated skill.name string allocations. Previously every inventory change triggered O(resources × tasks) recomputation with 2744+ string allocations per frame.
