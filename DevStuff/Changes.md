- UITicker performance optimizations: converted Subscribe/Unsubscribe from foreach to for loops (zero enumerator allocations), decoupled CauldronManager tick rate from roll rate (fixed 10Hz tick, rolls still at full rate via time-accumulation), added dirty flags and scratch lists to EnemyStatsPanelUI and TaskStatsPanelUI SortEntries (99% fewer sorts, zero LINQ allocations per tick). Expected 95% reduction in GC allocations from UITicker.Update().
