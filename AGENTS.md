...
- Ensure compatibility with **Unity 6000.5.1f1**, the version used by Echoes of Vasteria (Timeless Echoes).
- Do not create or commit `.meta` files unless absolutely required.
- Avoid obsolete Unity API calls. For scene queries, prefer explicit references; when a query is unavoidable, use `Object.FindAnyObjectByType` or the current `Object.FindObjectsByType` overload appropriate to the required inactive-object behavior.
