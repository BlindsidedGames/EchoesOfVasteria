...
namespace Gear
{
    public static class GearGenerator
    {
        public static GearItem Generate(int enemyLevel)
        {
            ...
            var statCount = GetStatCount(rarity);
            for (var i = 0; i < statCount; i++) AddRandomStat(item, enemyLevel, rarity);
            return item;
        }
        ...
        private static void AddRandomStat(GearItem item, int level, GearRarity rarity)
        {
            ...
            var mult = 1f + (int)rarity * 0.5f;
            switch (stat)
            {
                case 0:
                    if (item.damage == 0) item.damage = Mathf.CeilToInt(level * mult);
                    break;
                case 1:
                    if (item.attackSpeed == 0) item.attackSpeed = level * 0.05f * mult;
                    break;
                case 2:
                    if (item.health == 0) item.health = Mathf.CeilToInt(level * 2f * mult);
                    break;
                case 3:
                    if (item.defense == 0) item.defense = Mathf.CeilToInt(level * mult * 0.5f);
                    break;
                case 4:
                    if (item.moveSpeed == 0) item.moveSpeed = level * 0.1f * mult;
                    break;
            }
        }
        ...
        {
            var r = Random.value + level * 0.05f;
