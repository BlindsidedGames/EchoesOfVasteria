...
namespace TimelessEchoes.Gear
{
    public class HeroGear : MonoBehaviour
    {
        ...
        public int TotalDamage =>
            (ring?.damage ?? 0) + (necklace?.damage ?? 0) + (brooch?.damage ?? 0) + (pocket?.damage ?? 0);

        public float TotalAttackSpeed => (ring?.attackSpeed ?? 0) + (necklace?.attackSpeed ?? 0) +
                                         (brooch?.attackSpeed ?? 0) + (pocket?.attackSpeed ?? 0);

        public int TotalHealth =>
            (ring?.health ?? 0) + (necklace?.health ?? 0) + (brooch?.health ?? 0) + (pocket?.health ?? 0);

        public int TotalDefense => (ring?.defense ?? 0) + (necklace?.defense ?? 0) + (brooch?.defense ?? 0) +
                                   (pocket?.defense ?? 0);

        public float TotalMoveSpeed => (ring?.moveSpeed ?? 0) + (necklace?.moveSpeed ?? 0) + (brooch?.moveSpeed ?? 0) +
                                       (pocket?.moveSpeed ?? 0);
        ...
        private void SaveState()
        {
            ...
            if (dict == null)
                oracle.saveData.HeroGear = dict = new Dictionary<string, SaveData.HeroGearState>();
        ...
        {
            if (oracle == null) return;
            oracle.saveData.HeroGear ??= new Dictionary<string, SaveData.HeroGearState>();
