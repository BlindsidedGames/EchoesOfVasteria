...
namespace TimelessEchoes.EditorTools
{
    ...
    {
        [MenuItem("Tools/Save/ES3 Save Editor")]
        private static void Open()
        {
            var wnd = GetWindow<ES3SaveEditorWindow>(false, "ES3 Save Editor", true);
        ...
        [BoxGroup("Source")] [LabelText("Use Beta Prefix")]
        private bool beta;

        [BoxGroup("Source")] [ShowIf("beta")] [LabelText("Beta Iteration")] [MinValue(0)]
        private int betaSaveIteration;

        [BoxGroup("Source")] [LabelText("Slot")] [MinValue(0)] [MaxValue(2)]
        private int slot;

        // File-based loading
        [BoxGroup("Source")] [LabelText(".es3 File (absolute)")] [ReadOnly] [SerializeField]
        ...
        private PropertyTree gameDataTree;
        private bool gameDataDirty;

        // Arbitrary key value preview/edit
        private object selectedValue;
        private PropertyTree selectedValueTree;
        ...
        private void DrawToolbar()
        {
            using (new EditorGUILayout.HorizontalScope(EditorStyles.toolbar))
            {
                ...
                autoSaveOnEdit = GUILayout.Toggle(autoSaveOnEdit, "Auto Save", EditorStyles.toolbarButton,
                    GUILayout.Width(90));
                showRawJson = GUILayout.Toggle(showRawJson, "Raw JSON", EditorStyles.toolbarButton,
                    GUILayout.Width(90));
                if (GUILayout.Button("Open Save Folder", EditorStyles.toolbarButton, GUILayout.Width(140)))
                    OpenSaveFolder();
        ...
        private void DrawRawPane()
        {
            ...
            {
                rawJsonEdited = EditorGUILayout.TextArea(
                    string.IsNullOrEmpty(rawJsonEdited) ? rawJsonCache : rawJsonEdited, GUILayout.ExpandHeight(true));
            }
            ...
            {
                GUILayout.FlexibleSpace();
                rawEditable = GUILayout.Toggle(rawEditable, rawEditable ? "Editable" : "Read-only",
        ...
        private void DrawInspectorPane()
        {
            ...
            if (gameData != null)
            {
                ...
                {
                    gameDataTree = PropertyTree.Create(gameData);
            ...
            else if (selectedValue != null)
            {
                ...
                {
                    selectedValueTree = PropertyTree.Create(selectedValue);
                    selectedValueTree.UpdateTree();
                }

                EditorGUI.BeginChangeCheck();
                selectedValueTree.Draw(selectedValue is Object);
                if (EditorGUI.EndChangeCheck())
                {
        ...
        private void BrowseForFile()
        {
            ...
            if (!string.IsNullOrEmpty(path)) absoluteFilePath = path;
        }
        ...
        private void LoadAbsolute()
        {
            ...
            LoadFromSettings(loadedSettings, string.Empty, true);
        }
        ...
        private void LoadFromSettings(ES3Settings settings, string primaryKey, bool preferGameData)
        {
            try
            {
                ...
                try
                {
                    keys = ES3.GetKeys(fileSettings) ?? Array.Empty<string>();
                }
                catch
                {
        ...
        private ES3Settings ResolveEffectiveSettings(ES3Settings baseSettings)
        {
            return new ES3Settings(baseSettings.path, ES3.Location.File);
        }
        ...
        private void ApplyRawJson(bool saveAfter)
        {
            ...
            try
            {
                ...
                rawJsonCache = loadedFile.LoadRawString();
                rawJsonEdited = string.Empty;
                if (saveAfter) loadedFile.Sync(effective);
