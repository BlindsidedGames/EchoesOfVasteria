...
using TextUtils = VinTools.BetterRuleTiles.Editor.Utilities.TextUtils;

namespace VinTools.BetterRuleTiles.Editor.GUIWindows.BRTEditorWindows
{
    public class SpriteReplaceWindow : GUIWindow
    {

        public SpriteReplaceWindow(BetterRuleTileContainer container, BetterRuleTileEditor editor) : base(
        ...
        private static string FindCommonSpriteNameInSelection(BetterRuleTileContainer file, Rect selection, bool includeSubSprites)
        {
            ...
            foreach (var item in file._grid.Where(t => selection.Contains((Vector2)t.Position) && t.Sprite))
            {
        ...
        private static void ReplaceTiles(BetterRuleTileContainer file, Rect selection, int from, int to)
        {
            ...
            // replace
            foreach (var item in file._grid.Where(t => selection.Contains(t.Position) && t.TileID == file._tiles[from].UniqueID))
        ...
        private static void ModifySpriteData(BetterRuleTileContainer.UniversalSpriteData spriteData, BetterRuleTileContainer file, string from, string to)
        {
            ...
            // replace spritesAdd commentMore actions
        ...
        private static void CreateNewOverride(BetterRuleTileContainer.UniversalSpriteData spriteData, BetterRuleTileContainer file, string from, string to)
        {
            ...
            if (file._overrideSprites.Exists(d => d.BaseSprite == newSprite))
            {
                EditorUtility.DisplayDialog("Replace failed!", $"The sprite \"{newSprite.name}\" already has an override in the universal overrides window!", "ok");
                return;
            }
