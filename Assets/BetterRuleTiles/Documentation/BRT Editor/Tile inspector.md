...
The tile inspector is the place where you can change the settings of the **tiles** you created in the [[Better Rule Tile Editor#Tile drawer|tile drawer]] window. To edit a tile's settings, just select it from the tile drawer. After you selected the tile, the tile inspector window will automatically appear in the bottom left corner of the **editor window**.
...
- **Tile shape** - Here you can specify the shape of the tile. The default value of this is also affected by your current [[Better Rule Tile Editor#Grid|grid type]], so for square grids the default value will be **Square**, and for isometric grids, the default value will be **Isometric**. The shapes you can choose from are the following:
...
In the **tile variations** category, you can change whether that tile is unique or a variation of another tile. The default is unique, this is the default, and this way the tile will not be affected by other tiles. If the tile is not unique, it is a variation of another tile, which means it will fill all the missing [[Tiling rule|tiling rules]] which are not present in this tile, but present in the tile you specified in the **variation of** field, and add those [[Tiling rule|tiling rules]] to this tile.

- **Unique tile** - Is this tile unique or a variation of another tile?
- **Variation of** - The tile to fill in the missing [[Tiling rule|tiling rules]] from.

![[tile-variation.png]]

To see a specific example on how this feature works, look at the [[Examples#6 - Variations|examples]] provided with the package.
...
To see this behavior inside the tool, look at the [[Examples#4 - Connection|examples]] provided with the package.
...
![[tile-custom-properties 1.png]]

The properties you add here can be later accessed by scripts. Look at the [[Custom properties|custom properties]] page to learn more, or look at the [[Examples#7 - Custom properties|examples]] provided with the package.