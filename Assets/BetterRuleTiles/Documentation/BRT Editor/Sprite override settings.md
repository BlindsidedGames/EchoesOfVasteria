...
The [[Sprite override settings|sprite override settings]] window is where you can modify the output of a **sprite**, where you can assign gameobjects, set collider types, animations and more. If a [[Tiling rule|tiling rule]] corresponds to a sprite added to the overrides, the sprite output of that tile will be determined by the settings set here.

To open the window look for the **"Sprite override settings window"** button in the [[Better Rule Tile Editor#The toolbar|toolbar]] (Image with a gear icon). Clicking this will show the window. If the [[Better Rule Tile Editor#Sprite drawer|sprite drawer]] is visible, opening this window will hide the sprite drawer and show this window instead. You can also open the window by inspecting a sprite with the [[Editor Tools#Grid cell inspector tool|grid cell inspector]] tool and clicking the `Open sprite override settings` button. If the sprite is not in the overrides yet this button will say `Open sprite override settings`

![[show-override-settings.png]]

If you can't see these option in the toolbar you probably don't have the `Universal sprite settings` option enabled in the [[Better Rule Tile Editor#Export options|export options]]. After you've enabled this option the button will appear on the [[Better Rule Tile Editor#The toolbar|toolbar]].
# Adding sprites to overrides

To add sprites to the overrides you can either use the [[Editor Tools#Grid cell inspector tool|grid cell inspector]] tool or just simply drag and drop them in the overrides window.

If the `Universal sprite settings` option is enabled in the [[Better Rule Tile Editor#Export options|export options]], inspecting a sprite with the [[Editor Tools#Grid cell inspector tool|grid cell inspector]] tool will show an option to add the sprite to the overrides instead of showing the sprite options. If the sprite is already in the overrides the button will simply open the window, or select the sprite within the window if it's already opened.
...
To change the settings of a sprite, just simply click on them, which will reveal the settings for that sprite. Clicking a sprite will also select the [[Editor Tools#Brush tool|brush tool]] automatically and set the selected **sprite**, this way you don't have to switch to the [[Better Rule Tile Editor#Sprite drawer|sprite drawer]] to select a sprite which is also in this window.
