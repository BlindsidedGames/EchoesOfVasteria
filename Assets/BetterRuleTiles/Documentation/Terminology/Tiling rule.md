...
The concept of tiling rules in Better Rule Tiles is the same, with the only difference on how you place them down, for which you'll need the [[Editor Tools#Brush tool|brush]] tool.
- Place a **sprite** on a [[Better Rule Tile Editor#Tile drawer|tile]] to add that sprite to the [[Tiling rule|tiling rule]] of the tile you place it on.
...
The upside of having all tiles on one single large [[Better Rule Tile Editor#Grid|grid]] instead of multiple individual grids is that you can place [[Tiling rule|tiling rules]] directly next to each other. This means that you can draw out entire shapes using the sprites, than you can just draw tiles where sprites are.

> [!hint]
> Use the [[Editor Tools#Brush tool|brush]] tool to place sprites and tiles. You can see what is under a sprite using the **hide sprites** button on the [[Toolbar|toolbar]].
...
If you enable the **Simplify similar rules** option in the [[Better Rule Tile Editor#Export options|export options]], the tool will take care of creating those more generic tiling rules for you. The only downside of this option is that you'll need enough [[Tiling rule|tiling rules]] for the tool to simplify.
...
You can change this using the [[Editor Tools#Grid cell inspector tool|grid cell inspector]] tool under the `Neighbor positions`. This small grid is similar how you'd set the rules in a regular rule tile, but here it only determines where the tiling rule will check for it's neighbors. The actual rules are determined by the tiles you place on the [[Better Rule Tile Editor#Grid|grid]].

![[images/grid-cell-inspector.png]]

[[Preset block|Preset blocks]] use this feature to create larger structures. It just basically looks for all the neighbors that are part of the preset block, and if they're in the right configuration, the rules will match.
...
If you want your tile to always connect to another one like it was the same tile, you can set that up in the [[Tile inspector|tile inspector]] window under the `Connect to` category. The tile will connect to any tile you add there just like it'd connect to itself.

> [!hint]
> You can check the [[Examples|examples]] to see these features in action.
